...
shinyServer(function(input, output) {
    ...
    plotdata <- reactive( {
        if (input$dist=="Binomial") {
            ...
            if (input$poissonapprox) {
                data <- rbind(data,
                              data.frame(x=x, pmf=dpois(x, input$n*input$theta),
                                         cdf=ppois(x, input$n*input$theta),
                                         dist="Approximation by Poisson", stringsAsFactors=FALSE)
                              )                        # Add Poisson approx if needed
            }
        } else {
